...
export async function POST() {
  ...
  // 1. Fetch dynamic context (top items per stream).
  ...
  await Promise.all(
    FOCUS_CATEGORIES.map(async (category) => {
      const { data } = await db
        .from("tasks")
        .select(
          "title, priority, status, amount_usd, deadline, assigned_to",
        )
  ...
  const [
    ...
    db
      .from("inbox_items")
      .select("sender, subject, classification, urgency_score, received_at, category")
      .eq("status", "unread")
  ...
  try {
    output = await callClaude<BriefingOutput>({ task: "briefing", input });
  } catch (err) {
  ...
  await db.from("audit_log").insert({
    ...
    payload: { date: today, actor: user.email ?? user.id },
  });

  return NextResponse.json({ briefing: upserted });
}
